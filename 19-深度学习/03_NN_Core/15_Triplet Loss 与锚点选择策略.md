...
- **Easy triplets**：$D_{an} \geq D_{ap} + \alpha$，损失为 0，不参与训练。
- **Semi-hard triplets**：$D_{ap} < D_{an} < D_{ap} + \alpha$，负样本不比正样本近，但也不够远。
- **Hard triplets**：$D_{an} \leq D_{ap}$，负样本比正样本离锚点更近，是训练的重点。
