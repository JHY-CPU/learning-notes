...
**条件信息的注入方式**：
- **拼接 (Concatenation)**：将条件 $c$ 与 $x$ 或 $z$ 沿着特征维度拼接，最简单直接。
- **特征调制 (Feature Modulation)**：通过学习仿射变换（类似 AdaIN）将条件信息注入网络各层。
- **交叉注意力 (Cross-Attention)**：条件 $c$ 作为 Query/Key，特征图作为 Value，进行注意力计算（现代扩散模型的常用方式）。
