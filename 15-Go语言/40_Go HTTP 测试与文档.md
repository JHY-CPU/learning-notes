...
httptest 服务测试、表格驱动测试、mock 中间件、Golden 文件测试、OpenAPI/Swagger 文档生成。
...
//
// go test -covermode=count -coverprofile=count.out ./...  // 计数模式
```


> **Note:** 💡 HTTP 测试要点: httptest.NewRecorder + httptest.NewRequest 测试 handler; Gin TestMode + ServeHTTP; 表格驱动测试 (method/path/body/wantStatus); testify suite 组织测试; Golden 文件 testdata/*.golden 快照测试; httptest.Server 模拟外部 API; mock DB 注入测试; OpenAPI/Swagger 注解 @Summary/@Param/@Success/@Router; swaggo 自动生成文档; go test -cover 覆盖率。


## 练习


<!-- Converted from: 40_Go HTTP 测试与文档.html -->
