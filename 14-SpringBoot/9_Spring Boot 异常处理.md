...
@ControllerAdvice + @ExceptionHandler 全局异常处理、业务异常体系、验证错误处理、ErrorController/ApiError 统一响应。
...
## @ControllerAdvice 过滤器


```
// ========== @ControllerAdvice 作用范围 ==========
// 默认拦截所有 Controller
// 可通过参数限定范围
...
// ========== 7. 验证顺序 ==========
// 先声明式校验 (@Valid) → 业务异常 → 兜底异常
```


> **Note:** 💡 异常处理要点: @ControllerAdvice/@RestControllerAdvice 全局处理; @ExceptionHandler 指定异常类型; BusinessException 统一业务异常 (code/message/httpStatus); 参数校验 MethodArgumentNotValidException 返回字段级错误; 404/403/400/500 精准状态码; 兜底 Exception 避免未处理异常; 生产环境隐藏堆栈。


## 练习


<!-- Converted from: 9_Spring Boot 异常处理.html -->
