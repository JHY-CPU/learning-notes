...
def test_comparison():
    ...
    # SARSA
    Q_sarsa, _ = sarsa(env, num_episodes=500)
    # Q-Learning
    Q_qlearn, _ = q_learning(env, num_episodes=500)
    
    print("SARSA 策略（贴着安全区）:")
    print(np.argmax(Q_sarsa, axis=1).reshape(4, 12))
    print("\nQ-Learning 策略（贴着悬崖）:")
    print(np.argmax(Q_qlearn, axis=1).reshape(4, 12))
    ...
    print(f"\nSARSA 贪心评估: {evaluate(Q_sarsa, env)}")
    print(f"Q-Learning 贪心评估: {evaluate(Q_qlearn, env)}")

# 需要先定义 sarsa 函数（略），此处直接用 q_learning 的 results
...
1. **DQN = Q-Learning + 深度网络**：DQN (Deep Q-Network) 本质上是将 Q-Learning 的表格 Q 函数替换为深度神经网络。DQN 的损失函数 $L(\theta) = \mathbb{E}[(r + \gamma \max_{a'} Q(s', a'; \theta^-) - Q(s, a; \theta))^2]$ 与 Q-Learning 的更新目标完全一致。
2. **Off-policy 学习与经验回放**：Q-Learning 的 off-policy 特性是经验回放（Experience Replay）得以成功的关键。因为目标 $r + \gamma \max Q$ 不依赖行为策略，历史经验可以被反复使用。这使得深度学习可以在大规模数据上通过 mini-batch 训练。
3. **Double Q-Learning 到 Double DQN**：Q-Learning 的 $\max$ 操作会导致正偏差（overestimation bias）。Double Q-Learning 用两套 Q 表解耦选择和评估，Double DQN 将其推广到深度网络：使用在线网络选动作，目标网络评估价值。
