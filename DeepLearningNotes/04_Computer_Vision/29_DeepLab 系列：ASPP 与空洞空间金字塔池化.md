...
输入特征图 → 并行分支：
1. $1\times1$ 卷积（相当于 rate=0 的空洞卷积）
2. $3\times3$ 空洞卷积，rate=6
3. $3\times3$ 空洞卷积，rate=12
4. $3\times3$ 空洞卷积，rate=18
5. 全局平均池化 → $1\times1$ 卷积 → 双线性插值上采样回原尺寸
...
**DeepLab V3+的解码器：**
1. 编码器输出（ASPP特征）→ 4倍双线性上采样
2. 与骨干网络的低级特征（$1\times1$ 卷积降维到48通道）拼接
3. $3\times3$ 卷积细化特征
4. 4倍双线性上采样恢复到原始分辨率
