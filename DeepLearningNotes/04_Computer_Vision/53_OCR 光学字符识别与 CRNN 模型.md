...
1. **CNN特征提取**：输入 $I \in \mathbb{R}^{H \times W \times 3}$ → 输出特征图 $F \in \mathbb{R}^{H' \times W' \times D}$
2. **特征序列化**：按列划分 $X = [x_1, x_2, \dots, x_T]$，$T = W'$ 是序列长度
3. **BiLSTM**：$h_t = \text{BiLSTM}(x_t, h_{t-1})$，输出 $\{y_t\}_{t=1}^T$
