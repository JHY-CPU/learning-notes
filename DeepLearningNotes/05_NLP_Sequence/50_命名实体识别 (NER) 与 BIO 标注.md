...
**Bi-LSTM + CRF 模型**：
1. Bi-LSTM 编码：$\mathbf{h}_i = [\overrightarrow{\text{LSTM}}(x_i); \overleftarrow{\text{LSTM}}(x_i)]$
2. 发射分数：$E_{i, y} = W_{hy} \mathbf{h}_i + b_y$
3. CRF 解码：$P(\mathbf{y} | \mathbf{x}) = \frac{\exp(\sum_i E_{i, y_i} + \sum_i T_{y_{i-1}, y_i})}{\sum_{\mathbf{y}'} \exp(\sum_i E_{i, y'_i} + \sum_i T_{y'_{i-1}, y'_i})}$
