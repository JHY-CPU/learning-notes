...
- **NCCL 通信后端**：NVIDIA 提供的集体通信库，实现了 all-reduce、broadcast、all-gather 等操作的 GPU 间高效通信。在 GPU 集群训练中 NVDIA NCCL 是标准选择。
- **Ring All-Reduce 算法**：DDP 梯度同步的核心算法。将 N 个 GPU 组成逻辑环，每个 GPU 只与相邻 GPU 通信，分 scatter-reduce 和 all-gather 两个阶段完成全局梯度归约，通信复杂度为 O(N) 恒定的常数而与 GPU 数量无关。
