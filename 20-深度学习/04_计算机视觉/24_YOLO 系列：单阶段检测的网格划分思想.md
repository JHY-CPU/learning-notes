...
# 模拟YOLOv3的多尺度输出
x_large = torch.randn(2, 1024, 13, 13)   # 大物体分支（最深层，13x13）
x_medium = torch.randn(2, 512, 26, 26)   # 中物体分支
x_small = torch.randn(2, 256, 52, 52)    # 小物体分支（最浅层，52x52）

head_large = YOLOv3Head(1024)
head_medium = YOLOv3Head(512)
head_small = YOLOv3Head(256)

out_large = head_large(x_large)
out_medium = head_medium(x_medium)
out_small = head_small(x_small)

print(f"大尺度输出 (13x13): {out_large.shape}")   # (2, 13, 13, 3, 85)
print(f"中尺度输出 (26x26): {out_medium.shape}")  # (2, 26, 26, 3, 85)
print(f"小尺度输出 (52x52): {out_small.shape}")   # (2, 52, 52, 3, 85)
```
