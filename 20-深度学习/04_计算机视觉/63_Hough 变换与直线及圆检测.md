...
**Hough变换直线检测算法：**
- 对图像进行边缘检测（如Canny），得到边缘点集 $\{(x_i, y_i)\}$
- 初始化累加器 $A(\rho, \theta) = 0$
- 对每个边缘点 $(x_i, y_i)$，对每个 $\theta \in [0, \pi)$（离散化）：
   - 计算 $\rho = x_i\cos\theta + y_i\sin\theta$
   - $A(\rho, \theta) += 1$
- 在累加器 $A$ 中寻找局部最大值（投票峰值）
- 投票数超过阈值 $T$ 的 $(\rho_k, \theta_k)$ 对应检测到的直线
...
**圆的Hough梯度法（降低维度）：**
- 用Sobel计算每个边缘点的梯度方向 $\phi$
- 梯度方向指向圆心：圆心 $(a, b) = (x_i + r\cos\phi, y_i + r\sin\phi)$
- 对每个边缘点，沿梯度方向对 $r$ 进行投票
- 投票峰值对应检测到的圆
