...
- 将权重向量投影到单位球面上：$\hat{\theta}_A = \theta_A / \|\theta_A\|$, $\hat{\theta}_B = \theta_B / \|\theta_B\|$
- 计算两向量间夹角：$\Omega = \arccos(\hat{\theta}_A \cdot \hat{\theta}_B)$
- 球面插值：
...
- **修剪 (Trim)**：对每个权重参数，如果 $|\theta_A^{(i)} - \theta_B^{(i)}| < \tau$，认为该差异不显著，设为 0
- **符号投票 (Elect Sign)**：对每个任务 $k$ 的更新 $\Delta_k = \theta_k - \theta_{\text{base}}$，计算整体方向符号 $\gamma^{(i)} = \text{sign}\left(\sum_k \Delta_k^{(i)}\right)$
- **合并 (Merge)**：只在符号一致的方向上合并：$\theta_{\text{merged}}^{(i)} = \theta_{\text{base}}^{(i)} + \frac{1}{\sum_k \mathbb{1}[\text{sign}(\Delta_k^{(i)}) = \gamma^{(i)}]} \sum_{k: \text{sign}=\gamma} \Delta_k^{(i)}$
...
- **为什么不是简单平均**：深度学习模型的权重空间不是欧几里得的——两个优秀模型的均值可能是一个"四不像"。SLERP 和 TIES 通过考虑权重的方向和范数来避免这种情况。
