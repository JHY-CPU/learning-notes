...
- **压缩**：$z_0 = E(x)$，像素 $x$ 压缩为潜变量 $z_0$
- **扩散/去噪**：在潜空间中进行 DDPM 过程——$z_t = \sqrt{\bar{\alpha}_t} z_0 + \sqrt{1-\bar{\alpha}_t} \epsilon$，去噪网络 $\epsilon_\theta(z_t, t, \tau_\theta(c))$ 预测噪声
- **解压**：$\hat{x} = D(z_0)$，去噪后的潜变量 $z_0$ 解码为像素图像
