...
- **现代框架中的默认设置**：PyTorch 的 `nn.Linear` 默认使用 `$\mathcal{U}[-\sqrt{1/\text{fan_in}}, \sqrt{1/\text{fan_in}}]$` 初始化（源自 `kaiming_uniform` 但并非标准 He 初始化，He 的均匀形式范围是 `$\pm\sqrt{6/\text{fan_in}}$`）。在使用 Tanh 时，手动切换到 Xavier 初始化通常表现更好。
