...
- **Codebook Loss**：将码本向量 $e$ 拉向编码器输出 $z_e(x)$（只更新码本，不更新编码器）
- **Commitment Loss**：将编码器输出 $z_e(x)$"承诺"给码本（只更新编码器，不更新码本），$\beta$ 控制承诺强度（通常设为 0.25）
- **Reconstruction Loss**：标准的重建损失，通过直通估计器将梯度从解码器传回编码器
