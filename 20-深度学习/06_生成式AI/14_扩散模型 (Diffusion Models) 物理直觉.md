...
- **Classifier-Free Guidance (CFG)**：文生图扩散模型的核心技术。训练时随机以一定概率丢弃条件（如文本提示），使模型同时学会条件生成和无条件生成。推理时，最终噪声预测 = 无条件预测 + w × (条件预测 - 无条件预测)，其中 w > 1 的引导强度系数使生成结果更贴合条件描述。w 越大，生成图像越忠于文本但多样性降低；w=1 退化为标准条件生成。典型取值 w=7.5。
