...
- **压缩**：梅尔频谱图 $S \in \mathbb{R}^{F \times T}$ 通过 VAE 编码器压缩到潜空间 $z_0 = E(S)$
- **扩散/去噪**：在潜空间中执行条件扩散 $p_\theta(z_{t-1}|z_t, c)$，$c$ 是文本描述
- **解码为波形**：VAE 解码器恢复频谱图 $\hat{S} = D(z_0)$，声码器转换为波形 $w = \text{Vocode}(\hat{S})$
