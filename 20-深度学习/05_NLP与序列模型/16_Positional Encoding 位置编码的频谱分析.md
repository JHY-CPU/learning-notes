...
- **正弦-余弦位置编码**：使用不同频率的正弦和余弦函数为每个位置生成固定维度的编码向量。角频率从 $1$ 到 $10^{-4}$ 按几何级数分布。
- **频谱分析视角**：位置编码可以看作一组频率从高到低的信号。高频分量（周期短，$i=0$）编码局部位置关系，低频分量（周期长，高 $i$ 值）编码全局绝对位置。
...
- **位置编码像摩尔斯电码中的时间戳**：每个词的位置信息用一组"信号"编码。高频信号（短波）告诉你精确的局部偏移，低频信号（长波）告诉你大致在序列中的位置（前 1/4 还是后 1/4）。
- **不同频率的作用**：最高频的维度（$i=0$，周期约 $2\pi$）相邻几步就重复，编码局部/相对位置。最低频的维度（周期约为 $2\pi \times 10000 \approx 62832$）在整个训练长度内几乎单调递增，编码绝对位置。
...
# 查看不同频率的模式
print("高频维度 (i=0):", pe.pe[0, :10, 0].numpy())   # 快速振荡，周期约 2π
print("低频维度 (i=63):", pe.pe[0, :10, 63].numpy())  # 近乎线性增长
