...
- 当前解码器状态：$s_t = \text{RNN}(s_{t-1}, y_{t-1})$
- 对齐分数（三种变体）：
$$
e_t(s) = 
\begin{cases}
...
- 注意力权重：$\alpha_t(s) = \frac{\exp(e_t(s))}{\sum_{s'} \exp(e_t(s'))}$
- 上下文向量：$c_t = \sum_s \alpha_t(s) h_s$
- 注意力结合输出：$\tilde{h}_t = \tanh(W_c [c_t; s_t])$
- 预测：$P(y_t | y_{<t}) = \text{softmax}(W_s \tilde{h}_t)$
