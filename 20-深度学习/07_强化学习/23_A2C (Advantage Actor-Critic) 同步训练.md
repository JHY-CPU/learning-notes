...
- **并行训练与批归一化**：A2C 的多 worker 同步采集天然产生多样化的 batch 数据，类似于监督学习中大数据集上的 mini-batch SGD。这使 A2C 可以利用 GPU 批处理加速——将多个 worker 的状态拼接成一个 batch 进行前向传播，大幅提高吞吐量。
- **Entropy Regularization 的作用**：熵正则化在深度学习中类似于权重衰减或标签平滑——通过惩罚"过于自信"的预测来防止过拟合。在 DRL 中，它防止策略过早坍缩到次优的确定性策略，确保了持续的探索。
- **从 A2C 到 IMPALA**：A2C 的同步更新保证了数据与策略的一致性（on-policy），但约束了吞吐量。IMPALA (Importance Weighted Actor-Learner Architecture) 通过引入 V-trace 校正，解耦了 actor 和 learner，允许异步、高吞吐量的经验收集，同时保持 on-policy 的理论保证。
