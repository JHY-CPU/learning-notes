...
asyncpg PostgreSQL 驱动、aiosqlite、databases 库、异步 ORM (SQLAlchemy 1.4+ async)、连接池管理。
...
async def custom_types():
    ...
    print(row["data"]["key"])  # "value" 自动反序列化
```
...
> **Note:** 💡 异步数据库: asyncpg 高性能 PostgreSQL 驱动; aiosqlite 异步 SQLite; databases 统一接口; SQLAlchemy 1.4+ 支持 async; 连接池避免频繁创建连接。


## 练习


<!-- Converted from: 124_Python 异步数据库驱动.html -->
