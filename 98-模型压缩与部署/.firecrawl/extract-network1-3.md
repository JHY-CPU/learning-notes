# 运维工程师面试题总结-OpenVPN与JumpServer&云服务介绍15-16

URL: https://developer.aliyun.com/article/1344574
...
VPN是指在公共的网络上建立专用网络的技术，但是两个节点间并没有物理上的专用的端到端链路，而是通过 广域网或者运营商提供的网络平台之上的逻辑网络，用户数据在逻辑链路中传输，它可以有效的节省一般需要 达到DDN专线所能达到的同样的目的，而且VPN采用身份验证和加密技术，充分保证了安全性。常见的VPN有： ipsec vpn、PPTP vpn、L2TP vpn、SSL vpn，JumpServer是是一款使用 Python, Django 开发的开源跳板机系统, 为互联网企业提供了认证，授权，审计，自动化运 维等功能

# 2.ECS

云服务器（Elastic Compute Service，简称ECS）是阿里云提供的性能卓越、稳定可靠、弹性扩展的IaaS（Infrastructure as a Service）级别云计算服务。云服务器ECS免去了采购IT硬件的前期准备，像使用水、电、天然气等公共资源一样便捷、高效地使用服务器，实现计算资源的即开即用和弹性伸缩。阿里云ECS持续提供创新型服务器，解决多种业务需求，助力业务发展。

# 3.RDS

云数据库RDS是一种稳定可靠、可弹性伸缩的在线数据库服务，基于阿里云分布式文件系统和SSD盘高性能存储，RDS支持MySQL、SQL Server、PostgreSQL和MariaDB TX引擎，并且提供了容灾、备份、恢复、监控、迁移等方面的全套解决方案。同时阿里云在MySQL和PostgreSQL社区版的基础上，对内核进行了深度定制，提供了更多更强的功能，有效提升自建数据库的安全和可用性。云数据库RDS能够有效帮助客户提升数据库管理效率，降低数据库运维成本。
...
2.网络型负载均衡NLB（Network Load Balancer）：面向万物互联时代推出的新一代四层负载均衡，支持超高性能和自动弹性能力，单实例可以达到1亿并发连接，帮轻松应对高并发业务。NLB面向海量终端连接、高并发消息服务、音视频传输等业务场景针对性地推出了TCPSSL卸载、新建连接限速、全端口监听等高级特性，在物联网MQTTS加密卸载等场景为用户提供多种辅助手段，是适合IoT业务的新一代负载均衡。

3.传统型负载均衡CLB（Classic Load Balancer）：支持TCP、UDP、HTTP和HTTPS协议，具备良好的四层处理能力，以及基础的七层处理能力。

# 5.VPC

专有网络VPC（Virtual Private Cloud）是专有的云上私有网络。可以在自己定义的专有网络中使用阿里云资源，也可以在一个专有网络中创建多个交换机来划分子网。一个专有网络内的子网默认私网互通。

专有网络是独有的云上虚拟网络，可以将云资源部署在自定义的专有网络的交换机（子网）中。

交换机（vSwitch）是组成专有网络的基础网络设备，用来连接不同的云资源实例。专有网络是地域级别的资源，专有网络不可以跨地域，但包含所属地域的所有可用区。可以在每个可用区内创建一个或多个交换机来划分子网。
...
可以使用阿里云提供的API、SDK包或者OSS迁移工具轻松地将海量数据移入或移出阿里云OSS。数据存储到阿里云OSS以后，可以选择标准存储（Standard）作为移动应用、大型网站、图片分享或热点音视频的主要存储方式，也可以选择成本更低、存储期限更长的低频访问存储（Infrequent Access）、归档存储（Archive）、冷归档存储（Cold Archive）或者深度冷归档（Deep Cold Archive）作为不经常访问数据的存储方式。

# 7.弹性公网IP

弹性公网IP（Elastic IP Address，简称EIP）是可以独立购买和持有的公网IP地址资源。

目前，EIP支持绑定到专有网络类型的ECS实例、专有网络类型的私网SLB实例、专有网络类型的辅助弹性网卡、NAT网关和高可用虚拟IP上，当EIP和云资源绑定后，云资源可以通过EIP与公网通信

# 如果想上手操作的朋友们可以通过阿里云ecs服务器免费试用和低价购买，入口如下

# 入口一： [新人免费试用](https://free.aliyun.com/?userCode=2s3mpno6)

# 入口二： [大学生免费试用](https://www.aliyun.com/daily-act/ecs/activity_share?userCode=2s3mpno6)

# 入口三： [低价服务器购买](https://www.aliyun.com/activity/new/index?userCode=2s3mpno6)

# 入口四： [低价服务器购买2](https://www.aliyun.com/daily-act/ecs/activity_selection?spm=a2c6h.13046898.0.0.61546ffaeSJHFb&userCode=2s3mpno6)

# 入口五： [建站特惠购买](https://www.aliyun.com/minisite/goods?userCode=2s3mpno6)

文章标签：

[VPN网关](https://developer.aliyun.com/label/article_de-product-3-vpn)

[云服务器 ECS](https://developer.aliyun.com/label/article_de-product-3-ecs)

[弹性公网IP](https://developer.aliyun.com/label/article_de-product-3-eip)

[云数据库 RDS MySQL 版](https://developer.aliyun.com/label/article_de-product-3-mysql)

[对象存储](https://developer.aliyun.com/label/article_de-product-3-oss)

[负载均衡](https://developer.aliyun.com/label/article_de-product-3-slb)

[专有网络VPC](https://developer.aliyun.com/label/article_de-product-3-vpc)
...
[网络虚拟化](https://developer.aliyun.com/label/article_de-3-100114)
...
[运维云服务](https://www.aliyun.com/sswb/653380.html)

[运维工程师面试](https://www.aliyun.com/sswb/1367769.html)

[运维面试题](https://www.aliyun.com/sswb/553775.html)

[![](https://ucc.alicdn.com/avatar/be3faq7y2ftbo_78ea262798ee43b1be85894949529b10.jpg?x-oss-process=image/resize,h_150,m_lfit)](https://developer.aliyun.com/profile/be3faq7y2ftbo)
...
[魏红斌](https://developer.aliyun.com/profile/be3faq7y2ftbo)

+关注

[721文章](https://developer.aliyun.com/profile/be3faq7y2ftbo/article_1) [3201问答](https://developer.aliyun.com/profile/be3faq7y2ftbo/ask_1) [2视频](https://developer.aliyun.com/profile/be3faq7y2ftbo/live_1)
...
[热爱跑步的恒川](https://developer.aliyun.com/profile/escgmz5k5jf7y)

\|

[存储](https://developer.aliyun.com/label/sc/de-3-100262) [运维](https://developer.aliyun.com/label/sc/de-3-100073) [监控](https://developer.aliyun.com/label/sc/de-3-100072)

[云服务运维智能时代：阿里云操作系统控制台](https://developer.aliyun.com/article/1656245)
...
[Undoom](https://developer.aliyun.com/profile/iefdxf6pjkbf4)

\|

[弹性计算](https://developer.aliyun.com/label/sc/de-3-100063) [运维](https://developer.aliyun.com/label/sc/de-3-100073) [监控](https://developer.aliyun.com/label/sc/de-3-100072)

[云服务诊断工具深度评测：高效运维的利器](https://developer.aliyun.com/article/1648359)
...
[![云服务诊断工具深度评测：高效运维的利器](https://ucc.alicdn.com/pic/developer-ecology/iefdxf6pjkbf4_2a10ede7756c42ce82ea58ded7644e10.png?x-oss-process=image/format,webp/resize,h_160,m_lfit)](https://developer.aliyun.com/article/1648359)
...
[人工智能](https://developer.aliyun.com/label/sc/de-3-100052) [运维](https://developer.aliyun.com/label/sc/de-3-100073) [Prometheus](https://developer.aliyun.com/label/sc/de-3-100271)

[AIOpsLab：云服务自动化运维 AI，微软开源云服务 AI 框架，覆盖整个生命周期](https://developer.aliyun.com/article/1648347)
...
[![AIOpsLab：云服务自动化运维 AI，微软开源云服务 AI 框架，覆盖整个生命周期](https://ucc.alicdn.com/pic/developer-ecology/y4hwontyfrwnu_072ce18353b245b9a68c60e9ddceb6e4.png?x-oss-process=image/format,webp/resize,h_160,m_lfit)](https://developer.aliyun.com/article/1648347)
...
[存储](https://developer.aliyun.com/label/sc/de-3-100262) [网络协议](https://developer.aliyun.com/label/sc/de-3-100112) [安全](https://developer.aliyun.com/label/sc/de-3-100244)

[30 道初级网络工程师面试题，涵盖 OSI 模型、TCP/IP 协议栈、IP 地址、子网掩码、VLAN、STP、DHCP、DNS、防火墙、NAT、VPN 等基础知识和技术，帮助小白们充分准备面试，顺利踏入职场](https://developer.aliyun.com/article/1638453)
...
- [1\. 简述VPN，常见有哪几种？](https://developer.aliyun.com/article/1344574#slide-0)
- [2.ECS](https://developer.aliyun.com/article/1344574#slide-1)
- [3.RDS](https://developer.aliyun.com/article/1344574#slide-2)
- [4.SLB](https://developer.aliyun.com/article/1344574#slide-3)
- [5.VPC](https://developer.aliyun.com/article/1344574#slide-4)
- [6.OSS](https://developer.aliyun.com/article/1344574#slide-5)
- [7.弹性公网IP](https://developer.aliyun.com/article/1344574#slide-6)
- [如果想上手操作的朋友们可以通过阿里云ecs服务器免费试用和低价购买，入口如下](https://developer.aliyun.com/article/1344574#slide-7)
- [入口一：新人免费试用](https://developer.aliyun.com/article/1344574#slide-8)
- [入口二：大学生免费试用](https://developer.aliyun.com/article/1344574#slide-9)
- [入口三：低价服务器购买](https://developer.aliyun.com/article/1344574#slide-10)
- [入口四：低价服务器购买2](https://developer.aliyun.com/article/1344574#slide-11)
- [入口五：建站特惠购买](https://developer.aliyun.com/article/1344574#slide-12)
...
4008013260 [售前咨询](https://smartservice.console.aliyun.com/pre-sale/chat?entrance=201&referrer=https%3A%2F%2Fdeveloper.aliyun.com%2Farticle%2F1344574) [售后在线](https://smartservice.console.aliyun.com/service/robot-chat?entrance=201&referrer=https%3A%2F%2Fdeveloper.aliyun.com%2Farticle%2F1344574)
...
[快捷注册](https://account.aliyun.com/register/qr_register.htm?oauth_callback=https%3A%2F%2Fdeveloper.aliyun.com%2Farticle%2F1344574) [登录阿里云](https://account.aliyun.com/login/login.htm?oauth_callback=https%3A%2F%2Fdeveloper.aliyun.com%2Farticle%2F1344574)